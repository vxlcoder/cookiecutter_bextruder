...
class VIEW3D_OT_bextruder(CookieCutter):
    ...
    def update(self):
        ...
        self.segments = n
        self.extrude_map = {}
        self.extrude_map.update({ bmv:(i * (n + 1)) for (i, bmv) in enumerate(self.outer_verts) })
        self.extrude_map.update({ bmv:(l + i) for (i, bmv) in enumerate(self.inner_verts) })
        def m(v): return self.extrude_map[v]
        self.extrude_verts = [
            bmv.co + v * r / n
